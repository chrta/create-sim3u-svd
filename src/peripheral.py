...
class Peripheral:
    ...
    def _calc_xml_values(self):
        addr = functools.reduce(lambda x, reg: min(x, reg.address), self.registers.values(), 0xFFFFFFFF)          
    ...
    def xml_append(self, peripherals_element):
        ...
        ET.SubElement(p, 'description').text = self.description if self.description else "None"
